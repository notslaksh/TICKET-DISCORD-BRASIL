    ...
    run: async function(client, message, args) {
        try {
            ...
            if (!newPrefix) {
                ...
                message.channel.send({
                    embed: {
                        description: `O prefixo do bot mudou para**${require('../../config/bot').prefix}**`,
                        color: 0x00D700
                    }
                }).then(async function(msg) {
                    setTimeout(() => {
                ...
                if (newPrefix.length > 7) {
                    message.channel.send({
                        embed: {
                            color: 0xFF0000,
                            title: `**❌ | Error**`,
                            description: `Este prefixo é muito longo
`
                        }
                    }).then(async function(msg) {
                        setTimeout(() => {
                ...
                message.channel.send({
                    embed: {
                        description: `O prefixo do bot mudou para **${newPrefix}**`,
                        color: 0x00D700
                    }
                })
            }
