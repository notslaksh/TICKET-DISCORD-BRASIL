        ...
        try {
            ...
            if (checkTickets) {
                message.channel.send({
                    embed: {
                        color: 0xFF0000,
                        title: `**❌ | Error**`,
                        description: `Você já tem um ticket aberto antes`
                    }
            ...
            if (check == null || !check) {
                message.channel.send({
                    embed: {
                        color: 0xFF0000,
                        title: `**❌ | Error**`,
                        description: `você precisa configurar o sistema de tickets com este comando:\`${prefix}setup\``
                    }
                }).then(async function(m) {
                    setTimeout(() => {
                        m.delete()
            ...
            message.channel.send({
                embed: {
                    ...
                    description: `Tem certeza de que deseja criar um novo ticket?`,
                    color: 0xFFF200
                },
                component: row
